...
| 배틀 | **바탕화면에서 도트끼리 자동 전투.** 창도 체력바도 버튼도 없다 |
...
| 풀숲 | 클릭 = 살펴보기 |
| 야생 포켓몬 | **왼쪽 클릭 = 배틀 걸기** · **오른쪽 클릭 = 몬스터볼** |
...
client/
  ...
  poketdesktop/desktop_battle.py 바탕화면 배틀 진행
  poketdesktop/battle_fx.py 기술 연출 — 플래그+타입으로 12종
  poketdesktop/fx_layer.py  이펙트를 그리는 클릭 통과 레이어
...
python client/smoke_anim.py      # 바탕화면 배틀을 한 판 돌려서 찍어 확인
```
...
창을 띄우지 않습니다. 돌아다니던 내 포켓몬 한 마리가 야생 포켓몬 쪽으로
**걸어가서** 둘이 알아서 싸웁니다. 체력바도 기술 버튼도 없습니다.

플레이어가 하는 건 두 가지뿐입니다.

```
야생 포켓몬 왼쪽 클릭    배틀을 건다
야생 포켓몬 오른쪽 클릭   몬스터볼을 던진다  (배틀 중에 던지면 훨씬 잘 잡힌다)
```

기술은 서버가 고릅니다. 내 쪽은 제일 아픈 기술을 고르는 AI, 야생은 본가처럼
거의 무작위입니다. 무슨 일이 벌어지는지는 도트의 움직임과 이펙트, 그리고
"급소!" "효과가 굉장하다!" 같은 짧은 글씨로만 보여줍니다.
결과와 경험치는 트레이 알림으로 뜹니다.

이펙트는 **클릭이 그대로 통과하는 투명 레이어**에 그립니다.
그냥 투명색만 지정하면 그려진 부분이 클릭을 먹어서, 이펙트가 야생 포켓몬
위를 지날 때 오른쪽 클릭이 씹힙니다. `WS_EX_TRANSPARENT` 로 막았습니다.
