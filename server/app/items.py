...
BALL_NOTE = {
    # 이벤트 볼. **종 이름을 쓰지 않는다** - 가방에서 미리 읽히면 만났을
    # 때의 놀라움이 없어진다. 대신 등에 꽃이 핀 모습으로 일러 준다.
    # 만나 본 사람은 알아보고, 아직 못 만난 사람은 무엇을 찾아야 하는지만
    # 안다.
    "event_target": "등에 꽃이 핀 포켓몬에게만 통한다. 그 포켓몬은 반드시 잡는다",
