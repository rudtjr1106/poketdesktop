...
RAID_BOSS_LEVEL = _int("POKET_RAID_BOSS_LEVEL", 62)
# 보스 노력치 (능력마다). 참가자는 자기 노력치 510 을 다 들고 온다.
# **기준 두 개를 같이 맞춘다 - 잘 키운 파티(6V + 252 + 지닌 도구) 85%,
# 평균쯤 되는 팀 70%.** 유저가 '성능 좋은 애들을 너무 쉽게 깨면 안 된다' 고
# 정한 값이다.
#
# 예전 값(Lv.60 / 노력치 96 / 체력 2.0+1.6xN)은 잘 키운 파티가 94% 로 거의
# 안 졌다. 운영도 첫 나흘 8판 8승이었다. tools/sim_raid.py 인원당 300판:
#
#   보스        6V+생명의구슬 3/4/5/6인  평균    실제 운영 팀 3/4/5/6인  평균
#   Lv.60 ev96   88  92  96  99  94%          68  70  85  87  78%   <- 예전
#   Lv.62 ev192  87  80  86  93  86%          60  67  73  80  70%   <- 이것
#   Lv.60 ev252  84  80  86  93  86%          57  65  71  78  68%
#   Lv.63 ev192  82  78  87  91  84%          57  59  67  79  66%
#
# '실제 운영 팀' 은 레이드에 실제로 온 45명의 파티 그대로다 (개체값 평균
# 71%, 노력치 351/510, 도구 착용 75%). 이 사람들이 '평균쯤 되는 팀'이다.
#
# **재는 쪽이 중요하다.** sim_gym.trained_team 은 개체값을 안 굴려 평균 15 쯤
# 나온다 - 기준이 되는 6V 파티를 재려면 tools/sim_raid.py --perfect 를 쓴다.
RAID_BOSS_EV = _int("POKET_RAID_BOSS_EV", 192)
# 보스 체력 배율 = 기본 + 사람당. **기본을 0 으로 두고 사람 수에 비례시킨다.**
# 2.0+1.6xN 은 3인(6.8배)이 6인(11.6배)보다 1인당 체력을 많이 져서, 적은
