...
def main():
    ...
    try:
        ...
        if s != 200:
            ...
            # 여기서 rating == 1000 을 보면 안 된다. 상대는 서버가 고르는데,
            # 앞의 '랜덤 배틀' 에서 걸었던 c 가 걸리면 c 는 이미 1016 이다.
            # 그러면 delta 는 0 으로 멀쩡한데 절대값만 틀려서 세 번에 한
            # 번쯤 CI 가 헛되이 빨갛게 됐다. '그대로' 는 위 delta 로 본다.
